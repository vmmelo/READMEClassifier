...
def load_target_sections():
    ...
    # Extract heading
    overview = extractor.extract_headings_from_files_in_directory(readme_file_dir, db_filename,
                                                                  'target_section_overview')

    filenames = extractor.retrieve_readme_filenames_from_db(db_filename, 'target_section_overview')
    extractor.delete_existing_section_content_data(temp_abstracted_markdown_file_dir, db_filename,
                                                   'target_section_content')
    extractor.abstract_out_markdown(filenames, readme_file_dir, temp_abstracted_markdown_file_dir)
    extractor.extract_section_from_abstracted_files_v2(temp_abstracted_markdown_file_dir, db_filename,
